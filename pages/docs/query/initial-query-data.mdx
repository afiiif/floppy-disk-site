...
export default function Page({ popularProduct, productDetail, notifications }) {
  // Must be placed on the top or before any component subscribe to the query.
  // These setInitialResponse method will only run once.

  usePopularProductQuery
    .setInitialResponse({ response: popularProduct });

  useProductDetailQuery
    .setInitialResponse({ response: productDetail, key: { id: 3 } });

  useNotificationsQuery
    .setInitialResponse({ response: notifications, skipRevalidation: true });
