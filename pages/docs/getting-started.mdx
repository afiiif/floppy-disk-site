...
Create a global store for async data with `createQuery`:
...
function PlantDetail({ id }) {
  const usePlantDetailQuery = plantDetailQuery({ id: 1 });
...
import { type GetPlantByIdResponse } from "../api"; // Yours
